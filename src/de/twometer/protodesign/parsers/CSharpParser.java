...
public class CSharpParser implements IParser {
    ...
    public String process(String html) {
        ...
        return CodeScanner.scan(html, "csharp", (data, i, output) -> {
            String x = anyStartsAt(data, keywords, i);
            if (x != null && state.closeTag == -1 && (isValidKeywordStarter(data, i)) && isValidKeywordEnd(data, i, x) && !state.inComment()) {
                output.append(buildHtmlTag("csharp-keyword", true));
                state.closeTag = i + x.length() - 1;
            }
